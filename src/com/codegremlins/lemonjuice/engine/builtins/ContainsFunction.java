...
public class ContainsFunction extends FunctionElement {
    public ContainsFunction(Element[] parameters) {
        super(parameters);
    }
    
    @Override
    public Object evaluate(TemplateContext model) throws Exception {
        List ls = list(model, 0);
        
        if (parameters().length > 1) {
            for (int i = 1; i < parameters().length; i++) {
                Object value = parameters()[i].evaluate(model);
                if (!ls.contains(value)) {
                    return false;
                }
            }// FIXME: do own equality check, have contains all contains any
